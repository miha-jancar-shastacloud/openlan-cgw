...
impl CGWNBApiParsedMsg {
    ...
    async fn handle(self, server: Arc<CGWConnectionServer>) {
        match self.msg_type.clone() {
            CGWNBApiParsedMsgType::InfrastructureGroupCreate => {
                self.handle_infrastructure_group_create(server).await
            }
            CGWNBApiParsedMsgType::InfrastructureGroupCreateToShard(shard_id) => {
                self.handle_infrastructure_group_create_to_shard(server, shard_id).await
            }
            ...
            CGWNBApiParsedMsgType::InfrastructureGroupInfrasAdd(infras_list) => {
                self.handle_infrastructure_group_infras_add(server, infras_list).await
            }
            CGWNBApiParsedMsgType::InfrastructureGroupInfrasDel(infras_list) => {
                self.handle_infrastructure_group_infras_del(server, infras_list).await
            }
            CGWNBApiParsedMsgType::InfrastructureGroupInfraMsg(infra, msg, timeout) => {
                self.handle_infrastructure_group_infra_msg(server, infra, msg, timeout).await
            }
            CGWNBApiParsedMsgType::RebalanceGroups => {
                self.handle_rebalance_groups(server).await
            }
        }
    }

    async fn handle_infrastructure_group_create(&self, server: Arc<CGWConnectionServer>) {
        let gid = self.gid;
        let uuid = self.uuid;
        ...
        match server
            ...
            .await
            {
                ...
                Err(e) => {
                    warn!(
                        "Create group gid {}, uuid {} request failed! Error: {}",
                        gid, uuid, e
                    );
                    if let Ok(resp) = cgw_construct_infra_group_create_response(
                        gid,
    ...
    async fn handle_infrastructure_group_create_to_shard(&self, server: Arc<CGWConnectionServer>, shard_id: i32) {
        let gid = self.gid;
        let uuid = self.uuid;
        // DB stuff - create group for remote shards to be aware of change
        let group = CGWDBInfrastructureGroup {
    ...
    async fn handle_infrastructure_group_delete(&self, server: Arc<CGWConnectionServer>) {
        ...
        match server
            ...
            {
                Ok(()) => {
                    // We try to help free topomap memory usage by notifying it whenever GID gets destroyed.
                    // However, for allocation, we let topomap handle its mem alloc whenever necessary,
                    // on its own when data from a specific GID arrives - we rely on topomap to allocate
                    // necessary structures on its own. In this way, we make sure that we handle properly the
                    // GID restoration scenario: if CGW restarts and loads GID info from DB, there would be no
                    // notification about create/del, and there's no need to offload this responsibility to
                    // remote_discovery module for example, due to the fact that CGW is not designed for management
                    // of Redis without CGW knowledge: if something disrupts the Redis state / SQL state without
                    // CGW's prior knowledge, it's not a responsibility of CGW to be aware of such changes and handle
                    // it correspondingly.
    ...
    async fn handle_infrastructure_group_infras_add(&self, server: Arc<CGWConnectionServer>, infras_list: Vec<MacAddress>) {
        let local_shard_partition_key = {
            let kafka_key_r_lock = server.local_shard_partition_key.read().await;
            kafka_key_r_lock
        };
        ...
        if server.cgw_remote_discovery.get_infra_group_owner_id(gid).await.is_none() {
        ...
        match server.cgw_remote_discovery.create_infras_list(gid, infras_list.clone(), devices_cache_lock).await {
            Ok(success_infras) => {
                // Not all mac's GIDs might been successfully changed;
                // Notify all of them about the change.
                server.clone().notify_devices_on_gid_change(success_infras.clone(), gid);
                if let Ok(resp) = cgw_construct_infra_group_infras_add_response(
                    gid,
                ...
                for mac in success_infras {
                    ...
                    if queue_lock.check_messages_queue_exists(&mac).await {
                        queue_lock.set_device_queue_state(&mac, CGWUCentralMessagesQueueState::RxTx).await;
                    }
                    if let Some(dev) = devices_cache_read.get_device(&mac) {
                        if dev.get_device_state() == CGWDeviceState::CGWDeviceConnected {
                            ...
                            queue_lock.device_disconnected(&mac, dev.get_device_group_id()).await;
                        }
                    }
                }
            },
            Err(macs) => {
                if let Error::RemoteDiscoveryFailedInfras(failed_infras) = macs {
                    ...
                    if !macs_to_notify.is_empty() {
                        server.clone().notify_devices_on_gid_change(macs_to_notify, gid);
                    }
                    if let Ok(resp) = cgw_construct_infra_group_infras_add_response(
                        gid,
                        failed_infras,
    ...
    async fn handle_infrastructure_group_infras_del(&self, server: Arc<CGWConnectionServer>, infras_list: Vec<MacAddress>) {
        let local_shard_partition_key = {
            let kafka_key_r_lock = server.local_shard_partition_key.read().await;
            kafka_key_r_lock
        };
            ...
            .is_none()
        {
                ...
                .await
            {
                Ok(()) => {
                    ...
                    server.clone().notify_devices_on_gid_change(infras_list.clone(), 0i32);
                ...
                Err(macs) => {
                    if let Error::RemoteDiscoveryFailedInfras(failed_infras) = macs {
                        ...
                        // GID changes;
                        if !macs_to_notify.is_empty() {
                            server.clone().notify_devices_on_gid_change(macs_to_notify, 0i32);
                        }
    ...
    async fn handle_infrastructure_group_infra_msg(&self, server: Arc<CGWConnectionServer>, device_mac: MacAddress, msg: String, timeout: Option<u64>) {
        let local_shard_partition_key = {
            let kafka_key_r_lock = server.local_shard_partition_key.read().await;
            kafka_key_r_lock
        };
        ...
        if let Ok(parsed_cmd) = cgw_ucentral_parse_command_message(&msg.clone())
        {
            ...
            match devices_cache.get_device(&device_mac) {
                Some(infra) => {
                    ...
                    {
                        match server.config_validator.validate_config_message(
                            &msg,
                            infra.get_device_type(),
                        ) {
                            Ok(()) => {
                                // 3. Add message to queue
                                server.enqueue_infrastructure_request(
                                    (
                                        device_mac,
...
impl CGWConnectionServer {
    pub async fn new(app_args: &AppArgs) -> Result<Arc<Self>> {
        ...
        server.mbox_nb_api_runtime_handle.spawn(async move {
            server_clone
                .process_internal_nb_api_mbox(nb_api_rx)
                .await;
        });
    ...
    async fn kafka_partition_key_update(&self) {
        ...
        if !partition_mapping.is_empty() {
            ...
            debug!(
                ...
                partition_mapping[0].1,
                partition_mapping[0].0
            );
        } else {
        ...
        loop {
            if num_of_msg_read < buf_capacity {
                ...
                if rd_num == 0 {
                    ...
                    if last_update_timestamp != current_timestamp {
                        ...
                        for (infra, _) in connmap_r_lock.iter() {
                            if !self
                                .devices_cache
                                .read()
                                .await
                                .check_device_exists(infra)
                            {
            ...
            while !buf.is_empty() {
                ...
                if let CGWNBApiParsedMsgType::InfrastructureGroupCreate = parsed_msg.msg_type {
                    ...
                    continue;
                } else if let CGWNBApiParsedMsgType::InfrastructureGroupCreateToShard(_) = parsed_msg.msg_type {
                    parsed_msg.clone().handle(self.clone()).await;
                    // This type of msg is handled in place, not added to buf
                    // for later processing.
                    continue;
    ...
    async fn process_internal_mbox(self: Arc<Self>, mut rx_mbox: CGWConnectionServerMboxRx) {
        ...
        loop {
            ...
            while !buf.is_empty() {
                ...
                ) = msg
                {
                ...
                } else if let CGWConnectionServerReqMsg::ConnectionClosed(infra) = msg {
                    let mut device_group_id: i32 = 0;
                    info!(
                    ...
                    connmap_w_lock.remove(&infra);

                    let mut devices_cache = self.devices_cache.write().await;
                    if let Some(device) = devices_cache.get_device_mut(&infra) {
                        ...
                        if device.get_device_remains_in_db() {
                            ...
                            match serde_json::to_string(device) {
                                Ok(device_json) => {
                                    if let Err(e) = self
                                        .cgw_remote_discovery
                                        .add_device_to_redis_cache(&infra, &device_json)
                                        .await
                                    {
                                        error!("{e}");
                                    }
                            ...
                            devices_cache.del_device(&infra);
                            if let Err(e) = self
                                .cgw_remote_discovery
                                .del_device_from_redis_cache(&infra)
                                .await
                            {
                                error!("{e}");
                            }
                        ...
                        // Infra is not found in RAM Cache - try to get it frim PostgreSQL DB
                        if let Some(infra) = self.cgw_remote_discovery.get_infra_from_db(infra).await {
                            device_group_id = infra.infra_group_id;
                        }
                    }
                    ...
                    // Insert device to disconnected device list
                    {
                        let queue_lock = CGW_MESSAGES_QUEUE.read().await;
                        queue_lock
                            .device_disconnected(&infra, device_group_id)
                            .await;
                    }
                    ...
                    if self.feature_topomap_enabled {
                        let topomap = CGWUCentralTopologyMap::get_ref();
                        topomap
                            .remove_device(&infra, device_group_id, self.clone())
                            .await;
                    }
                    ...
                    let leave_message = match unassigned_infra_leave {
                        true => {
                            cgw_construct_unassigned_infra_leave_msg(infra, self.local_cgw_id)
                        }
